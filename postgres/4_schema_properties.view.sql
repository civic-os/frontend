CREATE
OR REPLACE VIEW public.schema_properties WITH (security_invoker = true) AS
SELECT
  ...
  COALESCE(
    ...
    initcap(
      replace(columns.column_name :: text, '_' :: text, ' ' :: text)
    )
  ) AS display_name,
...
FROM
  ...
  LEFT JOIN (
    ...
    FROM
      schema_relations_func() schema_relations_func(
        src_schema,
        src_table,
        src_column,
        constraint_schema,
  ...
  ) relations ON columns.table_schema :: name = relations.src_schema
  AND columns.table_name :: name = relations.src_table
  AND columns.column_name :: name = relations.src_column
  LEFT JOIN metadata.properties ON properties.table_name = columns.table_name :: name
  AND properties.column_name = columns.column_name :: name
WHERE
  columns.table_schema :: name = 'public' :: name
  AND (
    columns.table_name :: name IN (
      SELECT
        schema_entities.table_name
      FROM
        schema_entities
    )
  );
...
ALTER TABLE
  public.schema_properties OWNER TO postgres;

GRANT
SELECT
  ON TABLE public.schema_properties TO anon;

GRANT
SELECT
  ON TABLE public.schema_properties TO authenticated;

GRANT
SELECT
  ON TABLE public.schema_properties TO postgres;

GRANT
SELECT
  ON TABLE public.schema_properties TO service_role;
